...
def _update_stats(db: Session, user_id: int, seeds: list["_StatSeed"]) -> "_StatUpdateResult":
    ...
    for skill_code_lower, group in skill_groups.items():
        profile = db.scalar(
            select(UserSkillProfile).where(
                ...
                UserSkillProfile.skill_code == group[0].skill.strip(),
            )
        )
        ...
        profile.accuracy_pct = round(profile.correct_count * 100 / profile.attempt_count, 2) if profile.attempt_count else 0
    ...
    for part, group in part_groups.items():
        stat = db.scalar(select(UserPartStat).where(UserPartStat.user_id == user_id, UserPartStat.part == part))
        if stat is None:
            stat = UserPartStat(user_id=user_id, part=part, correct_count=0, attempt_count=0, updated_at_utc=datetime.utcnow(), average_time_seconds=0)
            db.add(stat)
        stat.correct_count += sum(1 for item in group if item.isCorrect)
        stat.attempt_count += len(group)
        stat.accuracy_pct = round(stat.correct_count * 100 / stat.attempt_count, 2) if stat.attempt_count else 0
    ...
    return _StatUpdateResult(skillStatsUpdated=skill_stats_updated, partStatsUpdated=part_stats_updated)
...
def _sync_user_profile(db: Session, user_id: int, latest_weak_skills_json: str | None = None) -> None:
    ...
    user.weak_skills_json = json.dumps(list(dict.fromkeys(item.strip() for item in weak_skills if item and item.strip())))
    db.commit()


def _build_practice_result(attempt_id: int, request: SavePracticeAttemptRequest) -> AttemptResultDto:
    questions = [_build_practice_result_question(item) for item in sorted(request.answers, key=lambda x: (x.questionNumber, x.questionId))]
...
def _build_mock_test_result(attempt_id: int, request: SaveMockTestAttemptRequest) -> AttemptResultDto:
    questions = [_build_mock_result_question(item) for item in sorted(request.answers, key=lambda x: (x.questionNumber, x.questionId))]
    total_questions = request.totalQuestions or len(questions)
    unanswered_count = max(total_questions - request.answeredCount, 0)
    wrong_count = max(request.answeredCount - request.correctCount, 0)
    return AttemptResultDto(
...
def _build_practice_result_question(answer: SavePracticeAttemptAnswerRequest) -> AttemptResultQuestionDto:
    return AttemptResultQuestionDto(
        ...
        explanation=_build_explanation(answer.explanation, answer.selectedAnswerIndex, answer.correctAnswerIndex, answer.selectedAnswerText, answer.correctAnswerText, answer.options),
        audio=answer.audio,
...
def _build_saved_result_question(answer, question_lookup: dict[str, ToeicRunnerQuestionDto]) -> AttemptResultQuestionDto:
    ...
    return AttemptResultQuestionDto(
        ...
        explanation=_build_explanation(answer.explanation, answer.selected_answer_index, answer.correct_answer_index, None, None, options),
...
def _build_skill_breakdown(questions: list[AttemptResultQuestionDto]) -> list[AttemptSkillBreakdownDto]:
    ...
    for key, group in grouped.items():
        ...
        result.append(AttemptSkillBreakdownDto(skill=key, total=total, correct=correct, accuracyPct=round(correct * 100 / total, 2) if total else 0))
...
def _build_part_breakdown(questions: list[AttemptResultQuestionDto]) -> list[AttemptPartBreakdownDto]:
    ...
    for part, group in grouped.items():
        total = len(group)
        correct = sum(1 for item in group if item.isCorrect)
        result.append(AttemptPartBreakdownDto(part=part, total=total, correct=correct, accuracyPct=round(correct * 100 / total, 2) if total else 0))
...
def _build_weak_areas(questions: list[AttemptResultQuestionDto]) -> list[AttemptWeakAreaDto]:
    ...
    weakest_skill = _build_weak_area_seed({key: value for key, value in _group_by_skill(questions).items()})
    if weakest_skill:
        result.append(AttemptWeakAreaDto(type="skill", label=weakest_skill.label, accuracyPct=weakest_skill.accuracyPct, total=weakest_skill.total, correct=weakest_skill.correct, suggestion=f"Ưu tiên luyện lại nhóm kỹ năng {weakest_skill.label} trước ở các câu tương tự."))
    subskill_groups = {key: value for key, value in _group_by_subskill(questions).items() if key}
    weakest_subskill = _build_weak_area_seed(subskill_groups)
    if weakest_subskill:
        result.append(AttemptWeakAreaDto(type="subskill", label=weakest_subskill.label, accuracyPct=weakest_subskill.accuracyPct, total=weakest_subskill.total, correct=weakest_subskill.correct, suggestion=f"Ôn lại subskill {weakest_subskill.label} và xem kỹ dấu hiệu nhận biết đáp án đúng."))
    part_groups = {f"Part {part}": value for part, value in _group_by_part(questions).items()}
    weakest_part = _build_weak_area_seed(part_groups)
    if weakest_part:
        result.append(AttemptWeakAreaDto(type="part", label=weakest_part.label, accuracyPct=weakest_part.accuracyPct, total=weakest_part.total, correct=weakest_part.correct, suggestion=f"Dành thêm thời gian luyện theo part {weakest_part.label.replace('Part ', '')} để cải thiện độ ổn định."))
    return result
...
def _build_weak_area_seed(groups: dict[str, list[AttemptResultQuestionDto]]) -> "_WeakAreaSeed | None":
    ...
    for label, group in groups.items():
        total = len(group)
        correct = sum(1 for item in group if item.isCorrect)
        items.append(_WeakAreaSeed(label=label, total=total, correct=correct, accuracyPct=round(correct * 100 / total, 2) if total else 0))
...
def _resolve_correct_answer_text(correct_answer: str | None, correct_answer_index: int | None, correct_answer_text: str | None, options: list[str] | None) -> str | None:
...
def _build_explanation(explanation: str | None, selected_answer_index: int | None, correct_answer_index: int | None, selected_answer_text: str | None, correct_answer_text: str | None, options: list[str] | None) -> str:
    ...
    if correct_answer_index is None:
        return "Chưa có giải thích chi tiết cho câu này." if selected is None else f"Bạn đã chọn {selected}. Chưa có giải thích chi tiết cho câu này."
    if selected_answer_index == correct_answer_index:
        return f"Bạn đã chọn đúng đáp án {correct}."
    return "Chưa có giải thích chi tiết cho câu này." if selected is None or correct is None else f"Bạn đã chọn {selected}. Đáp án đúng là {correct}."

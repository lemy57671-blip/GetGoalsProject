...
CT2_DIR = os.path.join(BASE_DIR, "ct2_model")
...
class Translator:
    ...
    def _load_model(self):
        ...
        if os.path.exists(CT2_DIR):
            ...
            print(f"[Translator] Loading CT2 model: {CT2_DIR}")
            self._tokenizer = MarianTokenizer.from_pretrained(FINETUNED_DIR if os.path.exists(FINETUNED_DIR) else MODEL_LOCAL)
            self._translator = ctranslate2.Translator(CT2_DIR, device="cpu", compute_type="int8")
        else:
            ...
            if os.path.exists(FINETUNED_DIR):
                ...
                path = MODEL_NAME
            print(f"[Translator] Loading HuggingFace model: {path}")
    ...
    def translate(self, text: str) -> str:
        ...
        # 3. Model
        if getattr(self, "_model_type", "hf") == "ct2":
    ...
    def translate_batch(self, texts: List[str]) -> List[str]:
        ...
        if need:
            ...
            if getattr(self, "_model_type", "hf") == "ct2":
                source_batch = [self._tokenizer.convert_ids_to_tokens(self._tokenizer.encode(k, padding=False)) for k in keys]
                results_ct2 = self._translator.translate_batch(
                    source_batch, 
                    max_decoding_length=MAX_LENGTH, 
                    beam_size=NUM_BEAMS, 
                    repetition_penalty=REPETITION_PENALTY
                ...
                for j, idx in enumerate(indices):
                    target = results_ct2[j].hypotheses[0]
                    r = self._tokenizer.decode(self._tokenizer.convert_tokens_to_ids(target), skip_special_tokens=True)
                    results[idx] = r

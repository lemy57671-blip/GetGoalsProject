...
def generate_for_current_user(db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    ...
    if roadmap is None:
        return JSONResponse(status_code=400, content={"message": "Unable to generate roadmap for this user."})
    return roadmap
...
def current(user_id: int, db: Session = Depends(get_db)):
    roadmap = get_current_roadmap(db, user_id)
    if roadmap is None:
        return JSONResponse(status_code=404, content={"message": "Active roadmap not found."})
    return roadmap
...
def week_sets(week_id: int, db: Session = Depends(get_db)):
    week = get_week_sets(db, week_id)
    if week is None:
...
def set_questions(week_id: int, set_id: int, db: Session = Depends(get_db)):
    payload = get_set_questions(db, week_id, set_id)
    if payload is None:
...
def start_week_route(week_id: int, db: Session = Depends(get_db)):
    week = start_week(db, week_id)
    if week is None:
        return JSONResponse(status_code=404, content={"message": "Roadmap week not found."})
    return week
...
def complete_week_route(week_id: int, db: Session = Depends(get_db)):
    week = complete_week(db, week_id)
    if week is None:
        return JSONResponse(status_code=404, content={"message": "Roadmap week not found."})
    return week

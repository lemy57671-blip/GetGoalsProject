...
def translate_batch(model, tokenizer, sentences, batch_size=16):
    ...
    for step, i in enumerate(range(0, len(sentences), batch_size)):
        ...
        percent = (step + 1) / total_batches * 100
        print(f"\r  Đang dịch batch: {percent:.1f}% ({step+1}/{total_batches})", end="", flush=True)
    print() # Xuong dong sau khi hoan thanh
    return results
